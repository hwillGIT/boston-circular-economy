...
export class LeafletProvider implements MapProvider {
  ...
  async showRoute(fromLat: number, fromLng: number, toLat: number, toLng: number): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(`https://router.project-osrm.org/route/v1/walking/${fromLng},${fromLat};${toLng},${toLat}?overview=full&geometries=geojson`)
      if (!response.ok) throw new Error('Route calculation failed')
      
      const data = await response.json()
      if (data.routes && data.routes.length > 0) {
        const coordinates = data.routes[0].geometry.coordinates.map((coord: [number, number]) => [coord[1], coord[0]]) // Convert back to lat,lng
